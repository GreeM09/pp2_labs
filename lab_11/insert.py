...
if __name__ == '__main__':   
    ...
    user_id = insert_users("Alihan", "Bahan", "kfc.kz@mail.ru", "Obshaga #3")
    if user_id:
        operator_id = 3
        insert_phone_numbers(user_id, operator_id, "+77772281337")
    
    user_id = insert_users("Nika", "Bahandi", "dotalegenda@gmail.kz", "228 Erubaev St")
    if user_id:
        operator_id = 2
        insert_phone_numbers(user_id, operator_id, "+77056671234")

    user_id = insert_users("Sanzhar", "Tokishev", "1234@inbox.com", "228 Street St")
    if user_id:
        operator_id = 5
        insert_phone_numbers(user_id, operator_id, "+77787881230") # real phone number

    user_id = insert_users("Alisher", "Aliev", "bezprichin09@mail.ru", "Homeless")
    if user_id:
        operator_id = 6
        insert_phone_numbers(user_id, operator_id, "+77777777777")

    user_id = insert_users("Balahan", "Marat", "priem@gmail.com", "Lesozavod")
    if user_id:
        operator_id = 4
        insert_phone_numbers(user_id, operator_id, "+77770688111") # real phone number

    user_id = insert_users("Alissa", "Borissova", "alo@mail.ru", "Obshaga #4")
    if user_id:
        operator_id = 3
